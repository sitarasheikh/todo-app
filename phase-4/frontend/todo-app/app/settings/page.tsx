  ...
  if (authLoading) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        ...
        >
          <div className="h-full w-full rounded-full border-4 border-var(--border)/20 border-t-var(--accent-settings) filter: drop-shadow(var(--shadow-sm))" />
        </motion.div>
      </div>
    );
  }
    ...
    <div className="min-h-screen py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <motion.button
          ...
          onClick={() => router.push("/")}
          className="mb-6 flex items-center gap-2 var(--accent-settings) hover:text-primary-300 font-medium transition-all duration-300 hover:gap-3 focus:outline-none focus:ring-2 focus:ring-var(--accent-settings)/50 rounded px-2 py-1"
        >
          <ArrowLeft size={20} />
          Back to Dashboard
        </motion.button>
        ...
        >
          <motion.div variants={fadeInUp} className="flex items-center gap-4">
            <div className="p-3 bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl box-shadow: var(--shadow-md)">
              <SettingsIcon className="h-8 w-8 var(--accent-settings) filter: drop-shadow(var(--shadow-sm))" />
            </div>
            <div>
              <h1
                className="text-4xl md:text-5xl font-bold text-white"
                style={{
                  background: 'linear-gradient(135deg, var(--accent-settings), #6366f1)',
        ...
        >
          ...
          <motion.div variants={listItem}>
            <GlassCard variant="elevated" className="p-6">
              ...
              >
                <Sun className="h-5 w-5 var(--accent-settings)" />
                Appearance
              </h2>
                ...
                <div className="flex items-center gap-1 p-1 bg-white/5 rounded-full border border-white/10">
                  <motion.button
                    ...
                    className={`p-2.5 rounded-full transition-all duration-300 ${
                      mode === 'light'
                        ? 'bg-gradient-to-r from-var(--accent-settings) to---accent-settings text-white shadow-[0_0_20px_rgba(139,92,246,0.5)]'
          ...
          <motion.div variants={listItem}>
            <GlassCard variant="elevated" className="p-6">
              ...
              >
                <User className="h-5 w-5 var(--accent-settings)" />
                Account
              </h2>
              ...
              <motion.button
                ...
                className="w-full flex items-center justify-between p-4 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 hover:border-var(--accent-settings)/30 transition-all duration-300 group"
                whileHover={{ x: 4 }}
              >
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-white/5 rounded-lg">
                    <Lock className="h-5 w-5 var(--accent-settings)" />
                  </div>
                ...
                <ChevronRight className="h-5 w-5 text-text-secondary group-hover:var(--accent-settings) transition-colors" />
              </motion.button>
            </GlassCard>
          </motion.div>
          ...
          <motion.div variants={listItem}>
            <GlassCard variant="elevated" className="p-6">
              ...
              >
                <Trash2 className="h-5 w-5 var(--accent-settings-muted)" />
                Data
              </h2>
