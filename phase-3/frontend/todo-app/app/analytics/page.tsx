  ...
  if (authLoading) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        ...
        >
          <div className="h-full w-full rounded-full border-4 border-var(--border)/20 border-t-var(--accent-analytics) shadow-glow-purple" />
        </motion.div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <motion.button
            ...
            onClick={() => router.push("/")}
            className="flex items-center gap-2 var(--text-secondary) hover:var(--text-secondary) font-medium transition-all duration-300 hover:gap-3 focus:outline-none focus:ring-2 focus:ring-var(--accent-analytics)/50 rounded px-2 py-1"
          >
            <ArrowLeft size={20} />
            Back to Dashboard
          </motion.button>
        ...
        >
          <motion.div variants={fadeInUp} className="flex items-center gap-4 mb-2">
            <div className="p-3 bg-[var(--bg-elevated)]/30 backdrop-blur-sm border border-[var(--border)]/30 rounded-xl box-shadow: var(--shadow-md)">
              <BarChart3 className="h-8 w-8 var(--accent-analytics) filter: drop-shadow(var(--shadow-md))" />
            </div>
            <div>
              <h1
                className="text-4xl md:text-5xl font-bold var(--text-primary)"
                style={{
              ...
              <p className="text-text-secondary mt-1 flex items-center gap-2">
                <TrendingUp size={16} className="var(--accent-analytics)" />
          ...
          >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <GlassCard variant="elevated" className="p-6">
                <h3
                  className="text-xl font-bold mb-4 var(--text-primary)"
                  style={{
                    background: 'linear-gradient(135deg, #c4b5fd 0%, #a78bfa 50%, #3b82f6 100%)',
                    WebkitBackgroundClip: 'text',
                    WebkitTextFillColor: 'transparent',
        ...
        {loading && !stats && (
          <div className="flex items-center justify-center py-20">
            <motion.div
              ...
              transition={{ duration: 1, repeat: Infinity, ease: "linear" }}
            >
              <div className="h-full w-full rounded-full border-4 border---accent-analytics/20 border-t---accent-analytics shadow-glow-blue" />
            </motion.div>
          </div>
        )}
      </div>
