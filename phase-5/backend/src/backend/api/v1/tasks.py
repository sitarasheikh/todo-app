...
async def mark_complete(
    ...
    """Mark task as completed with event publishing for recurring tasks (ownership verified)"""
    # Use async version to support event publishing (Phase 5 - recurring tasks)
    task = await TaskService.mark_complete_async(db, task_id, user_id)
    if not task:
        return error_response("Task not found", popup=None), status.HTTP_404_NOT_FOUND
