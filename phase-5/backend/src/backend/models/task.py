...
class Task(Base):
    ...
    # Recurring task fields (Phase 5)
    series_id = Column(UUID(as_uuid=True), ForeignKey("recurring_task_series.series_id", ondelete="SET NULL"), nullable=True, index=True)
    is_recurring = Column(Boolean, default=False, nullable=False)
    recurrence_pattern = Column(Text, nullable=True)  # Cached RRULE string for quick access
    next_occurrence = Column(DateTime(timezone=True), nullable=True)  # When next instance should be generated

    # Relationships
    user = relationship("User", back_populates="tasks")
    notifications = relationship("Notification", back_populates="task", cascade="all, delete-orphan")
    recurring_series = relationship("RecurringTaskSeries", back_populates="tasks")
